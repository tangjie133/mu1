...
#from mu.interface.themes import DAY_STYLE, CONTRAST_STYLE , NIGHT_STYLE
...
class ExampleItem(QListWidgetItem):
    """
    Represents an available mode listed for selection.
    """
...
class ExampleSelector(QDialog):
    """
    Defines a UI for selecting example
    """

    def setup(self, examples):
        self.setMinimumSize(600, 400)
        self.setWindowTitle(_('Select Example'))
        widget_layout = QVBoxLayout()
        label = QLabel(_('Please select the example then click "Open". '
                         'Otherwise, click "Cancel".'))
        label.setWordWrap(True)
        widget_layout.addWidget(label)
        self.setLayout(widget_layout)
        self.example_list = QListWidget()
        widget_layout.addWidget(self.example_list)
        self.example_list.setIconSize(QSize(48, 48))
        for name, item in examples.items():
            ExampleItem(name, item['description'], self.example_list)
        self.example_list.sortItems()
        button_box = QDialogButtonBox(QDialogButtonBox.Open |
                                      QDialogButtonBox.Cancel)
        button_box.accepted.connect(self.accept)
        button_box.rejected.connect(self.reject)
        widget_layout.addWidget(button_box)
    ...
    def get_example(self):
        ...
        """
        if self.result() == QDialog.Accepted:
            return self.example_list.currentItem().name
        else:
            raise RuntimeError('Mode change cancelled.')
