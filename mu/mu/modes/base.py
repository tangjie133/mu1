...
class FileManagerOs(FileManager):
    ...
    def ls(self):
        ...
        """
        try:
            result = tuple(os.listdir(self.disk))
            print("os list", result)
            self.on_list_files.emit(result)
        except Exception as ex:
            logger.exception(ex)
            self.on_list_fail.emit()
    ...
    def get(self, device_filename, local_filename):
        ...
        """
        try:
            srcFile = self.disk+device_filename
            shutil.copyfile(srcFile, local_filename)
            self.on_get_file.emit(device_filename)
        except Exception as ex:
            logger.error(ex)
            self.on_get_fail.emit(device_filename)
    ...
    def put(self, local_filename):
        ...
        """
        try:
            targetFile = self.disk+os.path.basename(local_filename)
            shutil.copyfile(local_filename, targetFile)
            self.on_put_file.emit(os.path.basename(local_filename))
        except Exception as ex:
            logger.error(ex)
            self.on_put_fail.emit(local_filename)
    ...
    def delete(self, device_filename):
        ...
        """
        try:
            targetFile = self.disk+device_filename
